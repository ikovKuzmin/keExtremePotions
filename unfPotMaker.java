...
public class unfPotMaker extends Script implements MessageListener,Paintable {
	...
	public class createUnfPot implements Strategy {
		...
		public void execute() {
			try{
				...
				if(bank!=null){
					Bank.open(bank);
					sleepBoolean=Game.getOpenInterfaceId() == bankInterface;
			...
			if(option==1){
				...
				if(Game.getOpenInterfaceId() == bankInterface && Bank.getCount(rangePot)>0){
					if(!Inventory.contains(grenwall) && Bank.getCount(grenwall)>5){
					...
					System.out.println("You are out of Range Potions (3) or Grenwalls");
				}
				
				if(Inventory.contains(grenwall) && Inventory.contains(rangePot)){
					Bank.close();
				...
				if(Game.getOpenInterfaceId() == bankInterface && Bank.getCount(extremeAtt)>0 && Bank.getCount(extremeStr)>0 
						&& Bank.getCount(extremeDef)>0 && Bank.getCount(extremeRange)>0
						&& Bank.getCount(extremeMagic)>0 && Bank.getCount(cleanTorstol)>0){
					...
					System.out.println("You are out of Extremes or clean torstols");
				}
				
				if(Inventory.contains(extremeAtt) && Inventory.contains(extremeStr)
						&& Inventory.contains(extremeDef) && Inventory.contains(extremeRange)
						&& Inventory.contains(extremeMagic) && Inventory.contains(cleanTorstol)){
					Bank.close();
					...
					sleep(sleepBoolean,6000);
				}
			}
			else if(option==7){
				if(Game.getOpenInterfaceId() == bankInterface && Bank.getCount(pestle_mortar)>0 && Bank.getCount(mudRune)>0){
					if(!Inventory.contains(pestle_mortar) && !Inventory.contains(mudRune)){
					...
					System.out.println("You are out of Magic Potions (3) or Ground Mud Runes");
				}
				...
				if(Inventory.contains(pestle_mortar) && Inventory.contains(mudRune)){
					...
					sleep(sleepBoolean,40000);
				}
			}
			else if(option==8){
				if(Game.getOpenInterfaceId() == bankInterface && Bank.getCount(overload3)>0){
					if(!Inventory.contains(overload3)){
						Bank.withdraw(overload3, 28, 100);
						sleepBoolean=Inventory.getCount(overload3)>0;
						sleep(sleepBoolean,1000);
					}
				}	
				else{
	...
	public static void createExtreme(final int potion, final int ingredient, final int extreme){
		if(Game.getOpenInterfaceId() == bankInterface && Bank.getCount(potion)>0 && Bank.getCount(ingredient)>0){
